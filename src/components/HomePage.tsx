...
const DesktopAirpodsCanvas = lazy(() =>
  import("./ui/desktop-airpods-canvas").then((module) => ({ default: module.DesktopAirpodsCanvas }))
);
...
function MobileHero() {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-bg-luxe">
      <div
        className="absolute inset-0 bg-cover bg-center opacity-90"
        style={{ backgroundImage: "url('https://i.postimg.cc/XYwvXN8D/img-4.png')" }}
      />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(5,5,5,0.10),rgba(5,5,5,0.64)_58%,rgba(5,5,5,0.9)_100%)]" />
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(5,5,5,0.18)_0%,rgba(5,5,5,0.08)_42%,rgba(5,5,5,0.86)_100%)]" />
      <div className="absolute inset-x-0 top-[38%] z-30 px-4 text-center uppercase">
        <div className="mx-auto max-w-sm rounded-[1.5rem] border border-white/10 bg-black/28 px-4 py-6 shadow-[0_24px_70px_rgba(0,0,0,0.42)] backdrop-blur-md">
          <h1 className="text-3xl font-black leading-tight tracking-tight text-white drop-shadow-[0_10px_34px_rgba(0,0,0,0.62)]">
            {titleString}
          </h1>
          <div className="mt-4 inline-flex max-w-full items-center rounded-full border border-white/20 bg-white/8 px-4 py-2 text-[11px] font-bold leading-relaxed tracking-[0.16em] text-white">
            {subtitle}
          </div>
        </div>
      </div>
      <div className="absolute inset-x-0 bottom-8 z-30 flex justify-center">
        <button
          ...
          className="flex h-20 w-20 items-center justify-center rounded-full border border-white/15 bg-black/20 text-center text-[9px] font-bold uppercase leading-tight tracking-[0.16em] text-white backdrop-blur-sm"
        >
          {t("Explore")}
        </button>
      </div>
...
export function HomePage() {
  ...
  const shouldUseLiteVisuals = shouldReduceMotion || isMobile;
  ...
  const handleAddToCartAnimation = (event: React.MouseEvent<HTMLElement>, product: any) => {
    ...
    window.dispatchEvent(
      new CustomEvent("triggerAddToCartAnimation", {
        detail: { product, startRect: rect, redirectToCart: true },
      })
    );
  };
              ...
              >
                ...
                <div className="absolute inset-[-10%] z-50 flex items-center justify-center md:inset-[-5%]">
                  {shouldUseLiteVisuals ? (
                    <img
                      src={
                        featuredProduct.image.includes("ik.imagekit.io")
                          ? `${featuredProduct.image}${featuredProduct.image.includes("?") ? "&" : "?"}tr=w-720,q-72,f-auto`
